...
function Bosuutap() {
  ...
  useEffect(() => {
    async function getProducts() {
      const collection = await fetch(`https://phantriptit.herokuapp.com/collection?slug=${slug}`).then(res => res.json());
      setUrlCollection(`collection=${collection[0].id}`);
  ...
  useEffect(() => {
    function getName(){   
      setWebLocation(`Bộ sưu tập ${nameCollection}`); 
    }
    getName();
  }, [nameCollection]);
