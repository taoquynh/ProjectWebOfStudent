...
function Sale() {
  ...
  // _page=${currentPage}&_limit=${pageLimit}
  const url = `https://phantriptit.herokuapp.com/products?${addUrl}${urlCollection}&&q=${search}&&_order=${order}&&_sort=${sort1}`
  ...
  function searchTag(event) {
    ...
    setCurrenPage(1);
    setUrlCollection("");
    setAddUrl(`_page=${currentPage}&_limit=${pageLimit}`);
    setWebLocation("Đồng hồ");
  }
  function showSort() {
    if (displaySort === 30) {
      setDisplaySort(-200);
  ...
  useEffect(() => {
    async function getProducts() {
      const collection = await fetch(`https://phantriptit.herokuapp.com/sales?slug=${slug}`).then(res => res.json());
      setUrlCollection(`sale=${collection[0].id}`);
      setNameCollection(collection[0].name);
      const products = await fetch(url).then(res => res.json());
      const allProducts = await fetch(`https://phantriptit.herokuapp.com/products?${urlCollection}&&q=${search}`).then(res => res.json());
  ...
  useEffect(() => {
    function getName(){   
      setWebLocation(`${nameCollection} Sale`); 
    }
    getName();
  }, [nameCollection]);
      ...
      <div className="filter-box" style={{ left: filterMobile }}>
          ...
          <div className="dong-ho-danh-sach" >
            ...
            <Link to="/"><p className="text2">Trang chủ&nbsp;</p></Link><img className="web-cocation-icon" src="https://phantri-techmaster.github.io/imgpamee/arrow.png" alt="" />
              <p className="text2 web-cocation-now">&nbsp;{webLocation}</p>
            </div>
            ...
            <Content handleAdd2={handleAdd2} products={products} totalPages={totalPages} handleFavorite2={handleFavorite2}
